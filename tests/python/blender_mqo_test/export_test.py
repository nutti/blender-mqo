...
class TestExportMqo(common.TestBase):
    ...
    def _is_same_mtrl(self, material_1, material_2, has_texture=False):
    ...
    def _is_same_object(self, obj_1, obj_2, ignore_keys={"_normals"}):
        ...
        for face_1, face_2 in zip(obj_1.get_faces(uniq=True),
            ...
            self.assertTrue(face_1.is_same(face_2, ignore_keys=ignore_keys))

        if obj_1.get_vertexattr('WEIT') is None:
            self.assertIsNone(obj_2.get_vertexattr('WEIT'))
        else:
            self.assertDictEqual(obj_1.get_vertexattr('WEIT'),
                                 obj_2.get_vertexattr('WEIT'))

    # pylint: disable=W0102
    def _is_same_mqo_file(self, mqo_file_1, mqo_file_2, has_texture=False,
                          object_ignore_keys={"_normals"}):
        ...
        for mtrl_1, mtrl_2 in zip(mqo_file_1.get_materials(),
            ...
            self._is_same_mtrl(mtrl_1, mtrl_2, has_texture=has_texture)
        ...
        for obj_1, obj_2 in zip(mqo_file_1.get_objects(),
            ...
            self._is_same_object(obj_1, obj_2, ignore_keys=object_ignore_keys)

    def test_export_mqo_no_object(self):
        export_filepath = "{}/no_object.mqo".format(EXPORTED_DIR)
    ...
    def test_export_mqo_single_object(self):
    ...
    def test_export_mqo_multiple_objects(self):
    ...
    def test_export_mqo_normal(self):
        ...
        export_filepath = "{}/normal.mqo".format(
            EXPORTED_DIR)
        bpy.ops.export_scene.blmqo_ot_export_mqo('EXEC_DEFAULT',
                                                 filepath=export_filepath,
                                                 add_export_prefix=False,
    ...
    def test_export_mqo_single_material(self):
        # TODO: because blender crash at bpy.ops.object.material_slot_assign(),
        #       we can not test in Blender 2.8
        if common.check_version(2, 80, 0) == 0:
            return
    ...
    def test_export_mqo_multiple_materials(self):
        # TODO: because blender crash at bpy.ops.object.material_slot_assign(),
        #       we can not test in Blender 2.8
        if common.check_version(2, 80, 0) == 0:
            return
    ...
    def test_export_mqo_texture(self):
        # TODO: because blender crash at bpy.ops.object.material_slot_assign(),
        #       we can not test in Blender 2.8
        if common.check_version(2, 80, 0) == 0:
            return
        ...
        export_filepath = "{}/texture.mqo".format(EXPORTED_DIR)
        bpy.ops.export_scene.blmqo_ot_export_mqo('EXEC_DEFAULT',
                                                 filepath=export_filepath,
                                                 add_export_prefix=False,
                                                 export_prefix="")
        ...
        self._is_same_mqo_file(export_mqo_file, import_mqo_file, True)

    def test_export_mqo_mirrored(self):
        ...
        export_filepath = "{}/mirrored.mqo".format(EXPORTED_DIR)
        bpy.ops.export_scene.blmqo_ot_export_mqo('EXEC_DEFAULT',
                                                 filepath=export_filepath,
                                                 add_export_prefix=False,
                                                 export_prefix="")
        ...
        for exp_obj, imp_obj in zip(export_mqo_file.get_objects(),
            ...
            self.assertTrue(exp_obj.get_faces(uniq=True)[0]
                            .is_same(imp_obj.get_faces(uniq=True)[0],
                                     ignore_keys={"_normals"}))

    def test_export_mqo_vertexattr(self):
        import_filepath = "{}/{}/vertexattr.mqo".format(
            os.path.dirname(os.path.abspath(__file__)), MQO_FILE_DIR)
        bpy.ops.import_scene.blmqo_ot_import_mqo(
            'EXEC_DEFAULT', filepath=import_filepath, add_import_prefix=False,
            import_prefix="", import_vertex_weights=True,
            vertex_weights_grouped_by='ALL_IN_ONE',
            vertex_weights_group_name="Vertex Weights Group")

        export_filepath = "{}/vertexattr.mqo".format(EXPORTED_DIR)
        vertex_groups = [
            {
                "name": "",
                "object_name": "obj1",
